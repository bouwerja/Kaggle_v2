...
siteName = input("What is the file name?")
multipleFiles = input("Do you have multiple files? [Y/n]")

if multipleFiles.lower() == "y":
    fileCount = input("How many files do you have?")
    fileCount = int(fileCount)
    fileNameList = []
    for i in range(0, fileCount - 1, 1):
        fileName = input(f"File {i} name: \n")
        fileNameList.append(fileName)

    for name in fileNameList:
        api.dataset_download_files(siteName, name)

elif multipleFiles.lower() == "n":
    fileName = input("File name: \n")
    api.dataset_download_file(siteName, fileName)
...
try:
    ...
    for file_path in all_files:
        ...
        try:
            ...
            print(
                "Problematic row:",
                data_to_insert[len(data_to_insert) - 1]
                if data_to_insert
                else "No rows to insert",
            )
            connection.rollback()
